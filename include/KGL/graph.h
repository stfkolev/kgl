...
//
//==========================================================================
// $Id: graph.h,v 1.43 2019/11/06 08:49:35 raitner Exp $
...
public:
    ...
    /**
     * Load graph from a file in KML-format. The optional
     * parameter <code>preserve_ids</code> controls whether to
     * give the nodes the same ids as in the KML file. You can enable this 
     * for debugging but you should disable it for final releases since 
     * it may make <code>node_map</code> unecessarily large.  
     * 
     * @param <code>filename</code> file in KML-format.  
     * @param <code>preserve_ids</code> if true all the nodes
     * will get the same id as in the KML file. If false (default) 
     * the nodes will be numbered consecutively beginning with 0. However 
     * the order of the nodes in the KML file will be preserved. 
     * @return detailed error description (hopefully KML_OK). For details 
     *   see @ref KML_error#err_num.
     */
    
    KML_error load (const string& filename, bool preserve_ids = false) 
    ...
    KML_error load (const char* filename, bool preserve_ids = false);

    /**
     * Save graph to file <code>filename</code> in KML-format, i.e.
    ...
    /**
     * Saves graph to stream <code>file</code> in KML-format.
     *
   ...
   virtual void post_clear_handler() {} 
     ...
     * @see graph#load 
     */
    virtual void top_level_key_handler (KML_pair* list);
     ...
     * @see graph#load 
     */
    virtual void load_node_info_handler (node n, KML_pair* list );
     ...
     * @see graph#load 
     */
    virtual void load_edge_info_handler (edge e, KML_pair* list);
     ...
     * @see graph#load 
     */
    virtual void load_graph_info_handler (KML_pair* list);
...
private:
    ...
    KGL_EXTERN friend ostream& operator<< (ostream& os, const graph& G);
};
...
#define forall_nodes(v,g) KGL_FORALL(v,g,graph::node_iterator,nodes_)
#define forall_edges(v,g) KGL_FORALL(v,g,graph::edge_iterator,edges_)
