...
//
//==========================================================================
// $Id: pq_tree.cpp,v 1.22 2019/05/07 18:12:07 chris Exp $

#include <KGL/pq_tree.h>
#include <KGL/debug.h>
...
{   
    ...
    KGL_debug::debug_message ("REDUCING %d\n", leaves.front()->id);
    fail = 0;
	...
	KGL_debug::debug_message ("Bubble-Up failed !!\n");
	...
	if (tmp->mark == pq_node::BLOCKED) {
	    ...
	    KGL_debug::debug_message ("creating pseudo-node as root\n");
	...
	if (tmp->pert_leaves == pert_leaves_count) {
		...
		pert_root = tmp;
		KGL_debug::debug_message ("full leaf is root\n");
	    } else if (tmp->kind() == pq_node::P_NODE) {
		if (P1 (tmp->P(), true)) {
		    KGL_debug::debug_message ("P1 matched for root\n");
		} else if (P2 (tmp->P())) {
		    KGL_debug::debug_message ("P2 matched for root\n");
		} else if (P4 (tmp->P())) {
		    KGL_debug::debug_message ("P4 matched for root\n");
		} else if (P6 (tmp->P())) {
		    KGL_debug::debug_message ("P6 matched for root\n");
		} else {
		    KGL_debug::debug_message ("NO MATCHING FOR P-ROOT\n");
		    fail = tmp;
		    failed_at_root = true;
		    return false;
		}
		...
		if (!tmp->Q()->pert_cons) {
		    KGL_debug::debug_message ("pertinent children not consecutive\n");
		    fail = tmp;
		    failed_at_root = true;
		    return false;
		} else if (Q1 (tmp->Q(), true)) {
		    KGL_debug::debug_message ("Q1 matched for root\n");
		} else if (Q2 (tmp->Q(), true)) {
		    KGL_debug::debug_message ("Q2 matched for root\n");
		} else if (Q3 (tmp->Q())) {
		    KGL_debug::debug_message ("Q3 matched for root\n");
		} else {
		    KGL_debug::debug_message ("NO MATCHING FOR Q-ROOT\n");
		...
		if (tmp->kind() == pq_node::LEAF) {
		    father->full (tmp->pos);
		    tmp->clear();
		    KGL_debug::debug_message ("full leaf processed\n");
		    
		} else if (tmp->kind() == pq_node::P_NODE) {
		    if (P1 (tmp->P(), false)) {
			KGL_debug::debug_message ("P1 matched for non-root\n");
		    } else if (P3 (tmp->P())) {
			KGL_debug::debug_message ("P3 matched for non-root\n");
		    } else if (P5 (tmp->P())) {
			KGL_debug::debug_message ("P5 matched for non-root\n");
		    } else {
			KGL_debug::debug_message ("NO MATCHING FOR P-NON-ROOT\n");
			fail = tmp;
			...
			KGL_debug::debug_message ("Q1 matched for non-root\n");
		    } else if (Q2 (tmp->Q(), false)) {
			KGL_debug::debug_message ("Q2 matched for non-root\n");
		    } else {
			KGL_debug::debug_message ("NO MATCHING FOR Q-NON-ROOT\n");
			fail = tmp;
			failed_at_root = false;
			return false;
		    }
...
void pq_tree::reset ()
{
	...
	tmp = clear_me.front();
	KGL_debug::debug_message ("Clearing %d\n", tmp->id);
	clear_me.pop_front();
	tmp->clear();
	tmp->pert_children = 0;
    }
...
KGL_EXTERN ostream& operator<< (ostream& os, const pq_tree& tree) 
{
...
{    
	...
	for (; it != end; ++it) {
		...
		if ((*it)->father != tmp) {
		    KGL_debug::debug_message ("Wrong father !!!\n");
 		    KGL_debug::close_debug();
		    return false;
		}
	    }
	    ...
	    if ((*it)->pos != it) {
		KGL_debug::debug_message ("Wrong position !!\n");
 		KGL_debug::close_debug();
		return false;
	    }
	...
	if (tmp->kind() == pq_node::P_NODE 
	    ...
	    KGL_debug::debug_message ("Wrong number of children !!!\n");
	    KGL_debug::close_debug();
	    return false;
	}
	
	if (tmp->kind() == pq_node::Q_NODE && count < 2) {
	    KGL_debug::debug_message ("Q-Node with too few children !!\n"); 
	    KGL_debug::close_debug();
	    return false;
	} 
	
	if (tmp->kind() == pq_node::P_NODE && count < 2) {
	    KGL_debug::debug_message ("P-Node with too few children !!\n");
	    KGL_debug::close_debug();
	    return false;
	}
	...
	if (tmp->kind() == pq_node::Q_NODE) {
		...
		if (!(tmp->sons.front()->is_endmost && 
		    ...
		    KGL_debug::debug_message ("Q-node with inner children labeled endmost\n");
		    KGL_debug::close_debug();
		    return false;
		} 
	    } else {
		KGL_debug::debug_message ("Q-node with too many or too few endmost children\n");
		KGL_debug::close_debug();
		return false;
	    }
	}
