...
//
//==========================================================================
// $Id: planarity.cpp,v 1.28 2019/05/07 18:12:07 chris Exp $

#include <KGL/planarity.h>
#include <KGL/pq_tree.h>
#include <KGL/biconnectivity.h>
#include <KGL/debug.h>
...
bool planarity::run_on_biconnected (graph& G, planar_embedding& em)
{
    ...
    if (G.number_of_edges() == 0) return algorithm::KGL_OK;
	...
	char buffer[12];
#ifdef __KGL_MSVCC
	_snprintf (buffer, 12, "%s%d.KML", filename, st_[curr]);
#else 
	snprintf (buffer, 12, "%s%d.KML", filename, st_[curr]);
#endif
...
#ifdef _DEBUG
	    os.open ("fail.KML", ios::out | ios::trunc);
	    os << PQ << endl;
	    os.close ();
#endif	    	
	    if (kup) {
...
#ifdef _DEBUG
	    KGL_debug::os() << "Embedding of " << st_[curr] << ":: ";
	    planar_embedding::iterator adit, adend;
	    for (adit = em.adj_edges_begin (curr), adend = em.adj_edges_end (curr); adit != adend; ++adit) {
		KGL_debug::os() << "[" << st_[curr.opposite (*adit)] << "]";
	    }
	    KGL_debug::os() << endl;
	    KGL_debug::os() << "Direction Indicators for: " << st_[curr] << ":: ";
...
{	   
...
#ifdef _DEBUG
	    KGL_debug::os() << "Component is: " << endl;
	    KGL_debug::os() << G << endl;
#endif
	...
	KGL_debug::debug_message ("graph is biconnected\n");
				     ...
				     pq_tree* PQ)
{
	...
	*/
#ifdef _DEBUG
	KGL_debug::os() << "Embedding so far (st_numbered): " << endl;
	em.write_st (KGL_debug::os(), st_);
#endif 
	...
	if (!q_fail->pert_cons) {
	    ...
	    KGL_debug::debug_message ("CASE C (non consecutive pertinent children)\n");
	    ...
	    KGL_debug::debug_message ("CASE D (non-root q-node with both endmost sons empty)\n");
		...
		KGL_debug::debug_message ("CASE D (non-root q-node with partial child at end of pertinent children\n");
		pq_tree::sons_iterator tmp = q_fail->sons.begin();
		pq_leaf* leaves[3];
		node nodes[3];
		leaves[0] = search_empty_leaf (*tmp);
		...
		KGL_debug::debug_message ("CASE C (q-node with partial children surrounded by pertinent children)\n");
		pq_tree::sons_iterator tmp = q_fail->pert_begin;
		pq_leaf* leaves[3];
		node nodes[3];
		leaves[0] = search_full_leaf (*tmp);
		...
		KGL_debug::debug_message ("CASE D (non-root q_node with first and last pertinent children partial)\n");
		...
		KGL_debug::debug_message ("CASE C (q_node with at least three partial children)\n");
	    ...
	    KGL_debug::debug_message ("CASE C (q_node with at least two partial children, at least one surrounded by pertinent)\n");
	    pq_tree::sons_iterator tmp = q_fail->pert_begin;
	    pq_leaf* leaves[3];
	    node nodes[3];
	    leaves[0] = search_full_leaf (*tmp);
	...
	if (p_fail->partial_count == 2) {
	    KGL_debug::debug_message ("CASE B (non-root p-node with two partial children)\n");
	    ...
	    KGL_debug::debug_message ("CASE A (p-node with at least three partial children)\n");
			...
			q_node* q_fail) 
{
	...
	KGL_debug::debug_message ("CASE E(i)\n");
	...
	KGL_debug::debug_message ("CASE E(ii)\n");
	...
	if (st_[tmp_nodes[1]] < st_[tmp_nodes[2]]) {
	    ...
	    KGL_debug::debug_message ("CASE E(iii) (1)\n");
	    ...
	    KGL_debug::debug_message ("CASE E(iii) (2)\n");
	    ...
	    KGL_debug::debug_message ("CASE E(iii) (3)\n");
	}
    }   
