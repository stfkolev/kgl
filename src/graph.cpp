...
//
//==========================================================================
// $Id: graph.cpp,v 1.58 2019/05/07 16:47:14 raitner Exp $

#include <KGL/graph.h>
#include <KGL/node_data.h>
#include <KGL/edge_data.h>
#include <KGL/node_map.h>

#include <KGL/dfs.h>
#include <KGL/topsort.h>
...
KGL_EXTERN ostream& operator<< (ostream& os, const graph& G) {
    node n;
    edge out;
    string conn;
...
KML_error graph::load (const char* filename, bool preserve_ids) {
    ...
    if (!file) {
	stat.err.err_num = KML_FILE_NOT_FOUND;
	return stat.err;
    } 

    KML_init ();
    key_list = KML_parser (file, &stat, 0);
    fclose (file);

    if (stat.err.err_num != KML_OK) {
	KML_free_list (key_list, stat.key_list);
	return stat.err;
    }
    
    //
    // This file is a valid KML-file, let's build the graph.
    // 
    ...
    KML_pair* graph_list = key_list;

    KML_pair* tmp_list;
    KML_pair* tmp_prev;
    // KML_pair* node_entries = 0;
    // KML_pair* edge_entries = 0;
    
    list<pair<int,KML_pair*> > node_entries;
    list<pair<pair<int,int>,KML_pair*> > edge_entries; 
    ...
    node tmp_node;
    edge tmp_edge;
    list<pair<int,KML_pair*> >::iterator it, end;
    vector<int> node_ids;
    ...
    list<pair<pair<int,int>,KML_pair*> >::iterator eit, eend;
    ...
    KML_free_list (orig_list, stat.key_list);
    stat.err.err_num = KML_OK;
    return stat.err;
}

void graph::load_node_info_handler (node n, KML_pair* li) {
}


void graph::load_edge_info_handler (edge e, KML_pair* li) {
}

void graph::load_graph_info_handler (KML_pair* li) {
}

void graph::top_level_key_handler (KML_pair* li) {
}
...
// void graph::top_level_key_handler (KML_pair_list::const_iterator it,
//     KML_pair_list::const_iterator end) 
// {
...
// void graph::load_graph_info_handler (KML_pair_list::const_iterator it,
//     KML_pair_list::const_iterator end)
// {
...
// void graph::load_node_info_handler (node n, KML_pair_list::const_iterator it,
//     KML_pair_list::const_iterator end)
// {
...
// void graph::load_edge_info_handler (edge e, KML_pair_list::const_iterator it,
//     KML_pair_list::const_iterator end)
// {
