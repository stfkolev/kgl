...
//==========================================================================
//
//   KML_scanner.cpp - Scanner for the KML - file format
//
//==========================================================================
// $Id: KML_scanner.cpp,v 1.10 2001/11/07 13:58:10 pick Exp $
...
unsigned int KML_line;
unsigned int KML_column;


int KML_search_ISO (char* str, int len) {
  ...
  for (i = 0; i < 96; i++) {
	if (!strncmp (str, KML_table[i], len)) {
	  ret = i + 160;
	  break;
	}
  }
...
struct KML_token KML_scanner (FILE* source) {
	...
	do {
		...
		token.value.err.err_num = KML_TOO_MANY_DIGITS;
		token.value.err.line = KML_line;
		token.value.err.column = KML_column + count;
		token.kind = KML_ERROR;
		return token;
	    }
	...
	do {
		...
		*tmp = 0;
		...
		token.value.err.err_num = KML_UNEXPECTED;
		token.value.err.line = KML_line;
		token.value.err.column = KML_column + count;
		token.kind = KML_ERROR;
	...
	*tmp = 0;
	...
	case '#':
	    ...
	    KML_line++;
	    KML_column = 1;
	    return KML_scanner (source);

	case '[':
	    token.kind = KML_L_BRACKET;
	    return token;

	case ']':
	    token.kind = KML_R_BRACKET;
	    return token;
	...
	case '"':
		...
		if (next == '&') {
		  ...
		  if (ISO_count) {
			...
			ISO_count++;
			next = KML_search_ISO (ISO_buffer, ISO_count);
			ISO_count = 0;
		  } else {
			next = '&';
		  }
		...
		*tmp++ = next;
		...
		if (next == EOF) {
		    token.value.err.err_num = KML_PREMATURE_EOF;
		    token.value.err.line = KML_line;
		    token.value.err.column = KML_column + count;
		    token.kind = KML_ERROR;
	...
	default:
	    token.value.err.err_num = KML_UNEXPECTED;
	    token.value.err.line = KML_line;
	    token.value.err.column = KML_column;
	    token.kind = KML_ERROR;
	    return token;
	}		
    }
